...
async function runMigration() {
  ...
  try {
    ...
    // Actualizar el campo 'related_badge_id' en 'activities' para referenciar a 'badges'
    try {
      await client.query(`
        ALTER TABLE activities 
        DROP CONSTRAINT IF EXISTS activities_related_badge_id_fkey;
        
        ALTER TABLE activities
        ADD CONSTRAINT activities_related_badge_id_fkey
        FOREIGN KEY (related_badge_id) REFERENCES badges(id);
      `);
    } catch (error) {
      console.log("No se pudo actualizar la restricción en activities. Posiblemente la tabla aún no existe.");
    }
